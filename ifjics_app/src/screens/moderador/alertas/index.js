...
export default class Alertas extends Component {
  ...
  async componentDidMount() {
    ...
    var alertas = await StorageService.getItem('alertas');

    if (alertas)
      this.setState({ listViewData: alertas, loaderVisible: false });
    else
      this.refresh();
  }
  ...
  async refresh() {
    ...
    await ApiService.get('/alertas')
      .then((response) => {
        ...
        StorageService.setItem('alertas', response.data);
  ...
  handleDelete = async () => {
    ...
    await ApiService.delete('/alertas/' + data.id)
      .then(() => {
    ...
    return (
      <Container>
        ...
        <Content padder>
          ...
          <Card>
            ...
            <CardItem>
              <Body>
                <Text note>
                  Aqui você tem acesso a lista de todos os alertas cadastrados no sistema.
                </Text>
              </Body>
            </CardItem>
          </Card>
            ...
            <List
              ...
              renderRow={(data, secId, rowId) =>
                <Card style={styles.mb}>
                  <CardItem header bordered>
                    <Left>
                      <Icon style={{ color: commonColor.secondary }} type='MaterialCommunityIcons' name='comment-alert' />
                      <Body>
                        <Text style={{ color: commonColor.primary }}>{data.nome}</Text>
                        <Text note>Alerta</Text>
                      </Body>
                    </Left>
                  </CardItem>
                  <CardItem>
              ...
              renderLeftHiddenRow={data =>
                <Button
                  ...
                  primary
                  full
                  onPress={() => this.props.navigation.navigate('AlertasEdit', { data: data, onRefresh: this.refresh.bind(this) })}
                  style={{
                    flex: 1,
                    alignItems: "center",
        ...
        <ActionButton buttonColor={commonColor.secondary}>
          <ActionButton.Item
            buttonColor="#1abc9c"
            title="Criar nova alerta"
            onPress={() => this.props.navigation.navigate('AlertasCreate', { onRefresh: this.refresh.bind(this) })}>
