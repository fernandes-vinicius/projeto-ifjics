...
export default class Edit extends Component {
  ...
  async componentDidMount() {
    ...
    var campi = data.campi.map(c => ({
      label: c.nome,
      value: c,
    }));
    var players = data.user.map(c => ({
      label: c.nome,
      value: c,
    }));
    var polos = data.polos.map(c => ({
      label: c.nome,
      value: c,
    }));
    this.setState({
    ...
    if (this.state.isModalidadeIndividual) {
      if (campi && campi.length > 0) {
        this.setState({ selectedList: campi, list: this.state.campus });
      } else if (polos && polos.length > 0) {
        this.setState({ selectedList: polos, list: this.state.polos });
      }
    }
    else {
      this.setState({ selectedList: players, list: this.state.players });
    }
  }
  ...
  async handleEdit() {
    if (!this.state.descricao.trim() || !this.state.data || !this.state.local.trim() || !this.state.hora.trim())
      ...
      const id = this.state.partida.id;
      const campi = [];
      const polos = [];
      if (this.state.selectedPickerCompeticao.modalidadeColetiva == 1) {
        ...
        await ApiService.put('/partida/' + id, {
  ...
  onSelectionsChangePlayer = (selectedList) => {
        ...
        <Content padder>
          ...
          <Card style={styles.mb}>
            <CardItem>
              <Body>
                <Text note>
                  Informe a descrição do jogo ou da rodada.
                </Text>
              </Body>
            </CardItem>
          ...
          <Card style={styles.mb}>
            <CardItem>
              <Body>
                <Text note>
                  Selecione a data dos(as) jogos/rodadas.
                </Text>
              </Body>
            </CardItem>
            ...
            <CardItem>
              <Left>
                ...
                <DatePicker
                  ...
                  animationType={"fade"}
                  androidMode={"default"}
                  placeHolderText={(this.state.data) ? this.state.data : 'dd/MM/yyyy'}
          ...
          <Card style={styles.mb}>
            <CardItem>
              <Body>
                <Text note>
                  Selecione todos os campi ou jogadores participantes do(a) jogo/rodada.
                </Text>
              </Body>
            </CardItem>
            ...
            <SelectMultipleComponent
              enableEmptySections
              items={this.state.list.map(c =>
                ({ label: ((c.campus != null && !this.state.isModalidadeIndividual) ? c.campus.nome : (c.polo != null) ? "Polo: " + c.polo.nome : c.nome), value: c, }))}
